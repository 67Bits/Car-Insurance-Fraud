	...
	public class MuscleCollisionBroadcaster : MonoBehaviour {
		...
		void OnCollisionEnter(Collision collision) {
			...
			foreach (BehaviourBase behaviour in puppetMaster.behaviours) {
				...
				{
					if (collision.gameObject.layer == 30 || collision.gameObject.layer == 4)
					{
						if (!character_movement.yachoquepiso)
						{
							ContactPoint contacto = collision.contacts[0];
							character_movement.feel.crarParticulaPolvo(contacto.point);
							character_movement.chocarConPiso();
						}
